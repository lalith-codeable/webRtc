...
const Userlist = ({
	...
	const RtcCall = async ({ recipient }: { recipient: string }) => {
		setIsCalling(true);
	...
	return (
		<>
			<div className="flex items-center space-x-6 pt-8 pl-8 pb-4">
				{/* Animated Logo */}
				<motion.div
					className="w-12 h-12 bg-[#bfc6dd] cursor-pointer rounded-full flex items-center justify-center shadow-lg"
			...
			>
				<h1 className="text-2xl ml-4 mb-2">Who's online 👀</h1>
				{Object.keys(users).map((uuid) => {
					return (
						users[uuid].username !== username && (
							<div
								className="w-full h-16 border-2 grid grid-cols-2 gap-10 px-2 rounded-md"
								key={uuid}
							>
								<span className="flex justify-center items-center ">
									<h2 className="w-full tracking-wide text-2xl">
										{users[uuid].username}
									</h2>
								</span>
								<span className="flex justify-end items-center ">
									<motion.button
										...
										whileTap={{ scale: 0.9 }}
										type="button"
										className={`text-xl  text-white font-bold  px-2 py-1 rounded-lg ${
											isCalling
												? "bg-slate-500"
												: "bg-green-600"
										}`}
